...
def wiener(input_img, PSF, eps, K = 0.01):
...
def CLSF(blurred, PSF, eps, gamma = 0.01):
...
if __name__ == '__main__':
    ...
    for gray in [b_gray, g_gray, r_gray]:
        ...
        inverse_limite_R_img[:, :, channel] = inverse_limite_R(blurred_img[:, :, channel], PSF, 0.001, 30)
        CLSF_img[:, :, channel] = CLSF(blurred_img[:, :, channel], PSF, 0.001)
        wiener_img[:, :, channel] = wiener(blurred_img[:, :, channel], PSF, 0.001)
        channel += 1
    cv2.imwrite('./img_data/blured_noise.jpg', cv2.merge([blurred_img[:, :, 0], blurred_img[:, :, 1], blurred_img[:, :, 2]]))
    cv2.imwrite('./img_data/wiener_noise.jpg', cv2.merge([wiener_img[:, :, 0], wiener_img[:, :, 1], wiener_img[:, :, 2]]))
    cv2.imwrite('./img_data/inverse_noise.jpg', cv2.merge([inverse_img[:, :, 0], inverse_img[:, :, 1], inverse_img[:, :, 2]]))
    cv2.imwrite('./img_data/inverse_limite_R_noise.jpg', cv2.merge([inverse_limite_R_img[:, :, 0], inverse_limite_R_img[:, :, 1], inverse_limite_R_img[:, :, 2]]))
    cv2.imwrite('./img_data/CLSF_noise.jpg', cv2.merge([CLSF_img[:, :, 0], CLSF_img[:, :, 1], CLSF_img[:, :, 2]]))
